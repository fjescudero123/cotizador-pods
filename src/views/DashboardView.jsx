...
<div class="foot"><p>MAYU · Cotizaci\u00f3n preliminar ${cotNum} · ${today}</p><p>Valores en CLP neto (sin IVA) · Sujeto a confirmaci\u00f3n de stock y plazos de entrega</p></div>
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto" style={{animation:'slideUp .3s ease'}}>
      ...
      <div className="bg-white p-6 rounded-2xl border shadow-sm"><h3 className="font-bold text-slate-700 mb-4">Tipologías</h3><div className="overflow-x-auto"><table className="w-full text-left text-sm min-w-[640px]"><thead><tr className="bg-slate-100 text-slate-600"><th className="p-3 border-b">Tipo</th><th className="p-3 border-b text-center">Un.</th><th className="p-3 border-b text-right">Área m²</th><th className="p-3 border-b text-right">M.O./POD</th><th className="p-3 border-b text-right">Costo total/POD</th><th className="p-3 border-b text-right">UF/m²</th><th className="p-3 border-b text-right">Utilidad/POD</th></tr></thead><tbody>{typs.map(t=>{const m=calc.typMetrics[t.id]||{};const area=m.floorArea||0;const matPP=m.materialCostPerPod||0;const labPP=m.laborCostPerPod||(t.config.laborCostPerPod||0);const costoPP=matPP+labPP;const ppod=m.salePricePerPod||0;const ufM2=area>0?ppod/area:0;const utilPP=ppod-costoPP;return(<tr key={t.id} className="border-b"><td className="p-3 font-semibold">{t.name}</td><td className="p-3 text-center font-bold text-blue-600">{t.count}</td><td className="p-3 text-right">{fmtN(area)} m²</td><td className="p-3 text-right">{fmtC(labPP)}</td><td className="p-3 text-right">{fmtC(costoPP)}</td><td className="p-3 text-right">{area>0?fmtUF(ufM2):'-'}</td><td className={`p-3 text-right font-semibold ${utilPP>=0?'text-emerald-600':'text-red-600'}`}>{fmtC(utilPP)}</td></tr>);})}</tbody></table></div></div>
    </div>
    {showQuote&&(
